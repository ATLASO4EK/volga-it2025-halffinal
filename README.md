...
Для запуска API локально (например, для использования `test_request.py`) необходимо:
...
Весь текущий исходный код скрипта для обработки видеопотока в RTCP находится в директории src/RTCP

Для запуска скриптов:\
- RTCP\
Установите и активируйте виртуальную среду Python 3.10.11\
Установите необходимые библиотеки (`pip install -r requirements.txt`)\
В файле src/RTCP/config.py измените RTSP_URL на ваш url камеры\
Запустите файл через терминал командой\
`python src/RTCP/main.py --model <Полный путь к файлу с моделью (best.pt)> --output <Полный путь к файлу с выходными данными (формат - .csv)>`\
После чего выполнится скачивание зависимостей (предобученных моделей) и программа запустится
- Video\
Установите и активируйте виртуальную среду Python 3.10.11\
Установите необходимые библиотеки (`pip install -r requirements.txt`)\
В файле src/RTCP/config.py измените RTSP_URL на ваш url камеры\
Запустите файл через терминал командой\
`python src/RTCP/videotest/main.py --video <Полный путь к файлу с видео> --model <Полный путь к файлу с моделью (best.pt)> --output <Полный путь к файлу с выходными данными (формат - .csv)>`
После чего выполнится скачивание зависимостей (предобученных моделей) и программа запустится