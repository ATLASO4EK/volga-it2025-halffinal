...
## Модель
В качестве модели для обучения была взята довольно мощная и одновременно легкая модель детекции объектов YOLOv11n.
...
[В разработке]\
Весь текущий исходный код скрипта для обработки видеопотока в RTSP находится в директории `src/RTCP`\
Более старый вариант кода находится в разделе `src/RTCP/Depricated`

Для запуска скриптов из раздела `src/RTCP/Depricated`:
- RTSP\
...
`python src/RTCP/Depricated/main.py --model <Полный путь к файлу с моделью (best.pt)> --output <Полный путь к файлу с выходными данными (формат - .csv)>`\
...
В файле src/RTCP/config.py измените RTSP_URL на ваш url камеры\
Запустите файл через терминал командой\
`python src/RTCP/Depricated/videotest/main.py --video <Полный путь к файлу с видео> --model <Полный путь к файлу с моделью (best.pt)> --output <Полный путь к файлу с выходными данными (формат - .csv)>`
После чего выполнится скачивание зависимостей (предобученных моделей) и программа запустится

Для запуска актуальных скриптов (`src/RTCP`):
- Настройте файл config.py
  - Укажите url камеры в переменной RTSP_url
  - Укажите True в переменной isVideo, если будете использовать локально хранящиеся видео или False, если нет
  - Укажите полный путь до файла с видео в переменной video_path
  - Укажите директорию, где будет создан файл с результатами в переменную output_folder
- Установите и активируйте виртуальную среду Python 3.10.11
- Установите необходимые библиотеки (`pip install -r requirements.txt`)
- Запустите скрипт main.py
  - Средствами IDE или
  - Через терминал (`python src/RTCP/main.py`)